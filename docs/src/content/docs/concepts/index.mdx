...
Crucially, it doesn't invent proprietary formats. `ksail cluster init` generates **native** configuration
...
```bash
ksail cluster init      # scaffold ksail.yaml + native configs + k8s/
...
[GitOps](https://opengitops.dev/) uses Git (or an OCI registry) as the single source of truth for declarative infrastructure and applications. Pick one engine per cluster at `ksail cluster init --gitops-engine`.
