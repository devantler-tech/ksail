...
- **Before calling `add_comment`, verify the target issue/PR exists**. If you already have an explicit issue or PR number, fetch that item directly with `issue_read` or `pull_request_read` instead of relying on open-only lists. Only use `list_issues` / `list_pull_requests` for existence checks when the listing includes closed/merged items as well. If the target does not exist or is not accessible, use `create_issue` or `noop` instead of commenting on a stale item number.
