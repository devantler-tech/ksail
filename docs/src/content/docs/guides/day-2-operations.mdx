...
| Check config drift before it bites | [`ksail cluster diff`](/cli-flags/cluster/cluster-diff/) — see [Drift Detection](/guides/cluster-provisioning/#drift-detection) |
...
The output is intentionally compact so it can be consumed by people *and* machines — the [AI chat assistant](/integrations/ai-chat/) and [MCP server](/integrations/mcp/) expose `diagnose` through the `cluster_read` tool and feed the findings back to the model as context for root-cause analysis.
...
- [Cluster Provisioning](/guides/cluster-provisioning/) — create, update, drift detection, and version upgrades
- [Backup & Restore](/guides/backup-restore/) — snapshot resources before risky changes
- [Troubleshooting](/troubleshooting/) — common issues and solutions
- [AI Chat Assistant](/integrations/ai-chat/) — let your configured AI provider interpret `diagnose` findings for you
