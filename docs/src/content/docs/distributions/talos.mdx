...
`ksail cluster update` automatically upgrades the Talos OS version when the Talos version bundled with KSail differs from the running cluster's version. KSail performs a rolling upgrade using the Talos LifecycleService API (Talos 1.13+):
...
> [!NOTE]
> Automatic upgrades require the running cluster to already be on Talos 1.13+. For clusters on older versions, first upgrade to 1.13 manually using the [Talos upgrade docs](https://www.talos.dev/latest/talos-guides/upgrading-talos/).

> [!NOTE]
> Omni-managed clusters are excluded from automatic upgrades — Omni handles OS upgrades externally through its own cluster lifecycle management.
