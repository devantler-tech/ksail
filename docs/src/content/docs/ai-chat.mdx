...
ksail cluster diagnose --format json   # machine-readable (healthScore + findings array)
```

The command works with Vanilla, K3s, Talos, and VCluster, and exits 0 even when failures are reported (non-zero is reserved for unreachable clusters).
