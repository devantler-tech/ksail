...
Press **Tab** to cycle between three modes, or use `/mode <name>` to switch directly:
...
When an MCP tool needs additional input during execution, an inline **Input Requested** modal appears with one or more form fields. Fill in the fields and press **Enter** to accept, or **Esc** to decline. Use **Tab / Shift+Tab** to navigate between fields in multi-field forms.

In non-TUI mode (plain terminal), elicitation prompts are presented as stdin prompts.
...
- See [Auto Model Selection](#auto-model-selection) to have Copilot automatically choose a model and billing tier.
- See [View Past Sessions](#view-past-sessions) to filter and resume past conversations.
- See [Reasoning Effort](#reasoning-effort) to balance thoroughness against response time.
...
**Authentication errors:** Authenticate Copilot CLI (`copilot auth login`) and use `KSAIL_COPILOT_TOKEN` or `COPILOT_TOKEN` environment variables for KSail chat authentication. KSail strips `GITHUB_TOKEN`, `GH_TOKEN`, and `COPILOT_GITHUB_TOKEN` from the child Copilot CLI process, so do not rely on them for `ksail chat` auth; they may still lack Copilot scopes or cause startup failures if you invoke `copilot` directly outside KSail. KSail automatically retries transient auth failures (HTTP 429, 500–504, network errors) up to 3 times with exponential backoff — `auth status check failed after N/3 attempts` messages indicate a persistent issue requiring action.
