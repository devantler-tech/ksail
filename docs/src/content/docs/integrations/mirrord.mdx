...
> [!NOTE]
> KSail sets the kubeconfig context after `ksail cluster create`. For context names by distribution, see [Companion Tools](/integrations/companion-tools/).
...
```bash
ksail project init --name dev --distribution K3s
...
```bash
kubectl --context k3d-dev create deployment my-app --image=nginx:latest --port=80 \
  --dry-run=client -o yaml | kubectl --context k3d-dev apply -f -
kubectl --context k3d-dev expose deployment my-app --port=80 \
  --dry-run=client -o yaml | kubectl --context k3d-dev apply -f -
```
...
> [!NOTE]
> mirrord uses your default kubeconfig (`~/.kube/config`) unless you set `kube_context` or point to a different file. If you have configured a custom kubeconfig path in KSail via `spec.cluster.connection.kubeconfig`, add `"kubeconfig": "<path>"` here to match.
...
|------|-----------|----------|
| `"mirror"` | Duplicate incoming traffic to both the pod and your local process | Observe and debug without affecting production behavior |
...
Replace `k3d-dev` with the context name for your distribution (see [Companion Tools](/integrations/companion-tools/)).
...
See [Companion Tools](/integrations/companion-tools/) for a full comparison of KSail's companion tool integrations.
