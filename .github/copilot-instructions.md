...
- **VSCode Cluster Status View**: Added real-time cluster status sidebar view (`vsce/src/views/clusterStatusView.ts`) and compact status bar indicator (`vsce/src/views/statusBar.ts`) to the VSCode extension (PR #2859); the sidebar polls every 10 seconds (configurable via `ksail.statusPollingInterval`) and shows cluster health (Healthy/Degraded/Error/Unknown), pod summaries grouped by namespace, and GitOps reconciliation state; clicking a failed/pending pod opens its logs in an output channel; the status bar item shows a color-coded health icon and is clickable to manually refresh
