...
2. **Gateway API experimental CRDs** — install from [gateway-api.sigs.k8s.io](https://gateway-api.sigs.k8s.io/guides/getting-started/#install-experimental-channel):
   ```bash
   kubectl apply -f https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.5.1/experimental-install.yaml
   ```
3. **Gateway controller** — one of: [Envoy Gateway](https://gateway.envoyproxy.io/), [Traefik](https://traefik.io/), [NGINX Gateway Fabric](https://docs.nginx.com/nginx-gateway-fabric/), [Contour](https://projectcontour.io/), or [HAProxy Ingress](https://haproxy-ingress.github.io/)
4. **GatewayClass** — must exist on the host cluster (created by the Gateway controller)
5. **Privileged pod policy** — nested cluster pods require `privileged: true` security context
...
Nested cluster CIDRs **must not overlap** with the host cluster's pod or service CIDRs. KSail validates this at configuration load time.
...
Each nested cluster runs in a dedicated namespace (`ksail-<cluster-name>`) on the host cluster. The execution model depends on the distribution:

| Distribution | Execution Mode | Description |
| --- | --- | --- |
| K3s | Direct pod | K3s binary runs directly in a privileged pod (k3k-style) |
| Vanilla (Kind) | Docker-in-Docker | Docker daemon sidecar, Kind SDK operates against it |
| Talos | Docker-in-Docker | Docker daemon sidecar, Talos SDK operates against it |
| VCluster | Docker-in-Docker | Docker daemon sidecar, VCluster Vind driver operates against it |
...
```bash
ksail cluster stop     # Delete node pods (namespace preserved)
ksail cluster start    # Recreate node pods
```
...
**`Gateway API experimental CRDs not found`** — Install the experimental CRDs: `kubectl apply -f https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.5.1/experimental-install.yaml`

**`gatewayClassName is required`** — Set `spec.provider.kubernetes.gatewayClassName` to a GatewayClass that exists on your host cluster. List available classes: `kubectl get gatewayclasses`
