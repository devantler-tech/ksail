...
Follow-up steps reach the cluster via `KUBECONFIG: ${{ steps.cluster.outputs.kubeconfig }}`. See [CI/CD Integration](/features/cicd-integration/) for the canonical action reference — common recipes, key inputs, and version-pinning guidance — and [PR Preview Clusters](/guides/pr-preview-clusters/) for PR preview patterns and GitOps CI workflows.
...
```bash
# Install the latest release (or pin VERSION to a specific release for reproducible pipelines;
# see https://github.com/devantler-tech/ksail/releases)
VERSION=$(curl -s https://api.github.com/repos/devantler-tech/ksail/releases/latest | jq -r '.tag_name | ltrimstr("v")')
