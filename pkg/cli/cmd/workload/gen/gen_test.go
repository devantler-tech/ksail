...
func genValidateCases() []genValidateCase {
	return []genValidateCase{
		...
		{"configmap", gen.NewConfigMapCmd, []string{"test-cm", "--from-literal=key=value"}},
		{
			"secret-generic",
			gen.NewSecretCmd,
			[]string{"generic", "test-secret", "--from-literal=password=test123"},
		},
		...
		{"job", gen.NewJobCmd, []string{"test-job", "--image=busybox:1.37"}},
		{
			"cronjob",
			gen.NewCronJobCmd,
			[]string{"test-cron", "--image=busybox:1.37", "--schedule=*/5 * * * *"},
		},
		{"serviceaccount", gen.NewServiceAccountCmd, []string{"test-sa"}},
		{
			"clusterrolebinding", gen.NewClusterRoleBindingCmd,
			[]string{"test-crb", "--clusterrole=test-cr", "--serviceaccount=default:test-sa"},
		},
		{
			"rolebinding",
			gen.NewRoleBindingCmd,
			[]string{"test-rb", "--role=test-role", "--serviceaccount=default:test-sa"},
		},
		...
		{
			"poddisruptionbudget", gen.NewPodDisruptionBudgetCmd,
			[]string{"test-pdb", "--min-available=1", "--selector=app=test"},
		},
		{
			"helmrelease", gen.NewHelmReleaseCmd,
...
func TestGenAndValidate(t *testing.T) {
	...
	for _, testCase := range genValidateCases() {
		t.Run(testCase.name, func(t *testing.T) {
			...
			output, _, err := execGen(t, testCase.cmdFactory, testCase.args)
			require.NoError(t, err)
			require.NotEmpty(t, output)

			tmpDir := t.TempDir()
			manifestPath := filepath.Join(tmpDir, testCase.name+".yaml")
