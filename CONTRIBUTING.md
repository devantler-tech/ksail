...
> **Tip:** `make generate` runs mockery together with every other generator in the repository — see [Auto-generated artifacts](#auto-generated-artifacts).
...
Several files are generated from source and should **not** be edited manually. **`make generate` is THE regeneration command** — it runs every generator in dependency order:
...
The individual generators, for reference (only needed when you want to run a single one):

- **Configuration schema:** `schemas/ksail-config.schema.json` — `go generate ./schemas/...`
- **CRD & deepcopy:** `charts/ksail-operator/crds/` and `pkg/apis/cluster/v1alpha1/zz_generated.deepcopy.go` — `go generate ./pkg/apis/...`
- **CLI flags reference** (`docs/src/content/docs/cli-flags/`, from the Cobra command tree) **and configuration reference** (`docs/src/content/docs/configuration/declarative-configuration.mdx`, from v1alpha1 types) — `go generate ./docs/...`
- **Chat assistant docs:** `pkg/svc/chat/docs_generated.go` — `go generate ./pkg/svc/chat/...` (reads the generated reference docs, so run `go generate ./docs/...` first)
- **Mocks:** `mocks.go` files — `mockery`
- **Web UI config types:** `web/ui/src/generated/ksail-config.ts` — `npm --prefix web/ui run gen:types` (reads the JSON schema, so run `go generate ./schemas/...` first)

CI's generate job regenerates all of these (except mocks, which only change when `.mockery.yml` or mocked interfaces change) on pull requests and auto-commits any drift (fork PRs fail with instructions instead), so stale generated files never land on `main`.
...
- **Talos ISO** – A Talos Linux ISO must be available in your Hetzner Cloud project. KSail tracks Hetzner's latest compatible public x86 ISO from the official changelog; verify that default under **Images → ISOs** in the Hetzner Cloud Console or configure another available ISO explicitly.
