import { Boxes, Lock, LogOut, Moon, Server, Sun } from "lucide-react";
...
function NavItem({ icon, label, active }: { icon: ReactNode; label: string; active?: boolean }) {
  return (
    <span
      className={
        active
          ? "flex items-center gap-2.5 rounded-md bg-blue-50 px-3 py-2 text-sm font-medium text-blue-700 dark:bg-blue-500/10 dark:text-blue-400"
          : "flex items-center gap-2.5 rounded-md px-3 py-2 text-sm font-medium text-slate-600 dark:text-slate-400"
      }
    >
      {icon}
      {label}
    ...
    <div className="flex h-full">
      <aside className="hidden w-60 shrink-0 flex-col border-r border-slate-200 bg-white md:flex dark:border-slate-800 dark:bg-slate-900">
        ...
        <nav className="flex-1 space-y-1 p-3">
          <NavItem icon={<Server className="size-4" aria-hidden />} label="Clusters" active />
        </nav>
