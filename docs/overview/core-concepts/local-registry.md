...
Using a `Local Registry` will create an official `registry:2` container in a specified container engine. The registry is configured to be accessible on `localhost`.
