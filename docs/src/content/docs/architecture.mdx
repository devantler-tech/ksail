...
1. **API server stability** — requires consecutive successful health checks within a 2-minute timeout. The threshold is distribution-aware: 3 checks for Vanilla and K3s (which stabilize faster after webhook registration), and 5 for Talos, VCluster, and KWOK.
