...
Creating a cluster **requires** this file; inspection-only operations (`list`, `info`, `start`, `stop`, `delete`) work without it. Unknown fields are rejected, so the file stays in lockstep with the GKE API. Store it next to `ksail.yaml`, or point `spec.cluster.distributionConfig` at a custom path.
