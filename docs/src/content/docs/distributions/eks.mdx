...
- The control plane keeps running (and billing) while nodegroups are stopped — use `ksail cluster delete` to stop all charges
- KSail-managed component installation (CNI/CSI/GitOps/security add-ons beyond eksctl's built-ins) and `ksail cluster update` for EKS are still in progress; see [Support Matrix](/support-matrix/)
