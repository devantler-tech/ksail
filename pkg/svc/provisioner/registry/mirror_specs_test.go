...
func TestMirrorSpec_ResolveCredentials_EnvVars(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			spec: registry.MirrorSpec{ //nolint:gosec // G101: test uses env var placeholder strings, not hardcoded credentials
				Host:     "ghcr.io",
				Remote:   "https://ghcr.io",
				Username: "${TEST_USER}",
				Password: "${TEST_TOKEN}",
...
func TestParseMirrorSpecs_AtSignInURL(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expected: []registry.MirrorSpec{
				{ //nolint:gosec // G101: test fixture with URL-embedded credentials for parsing verification
					Host:     "docker.io",
...
func TestBuildRegistryInfosFromSpecs_WithCredentials(t *testing.T) {
	...
	specs := []registry.MirrorSpec{
		{ //nolint:gosec // G101: test uses env var placeholder strings, not hardcoded credentials
			Host:     "ghcr.io",
			Remote:   "https://ghcr.io",
