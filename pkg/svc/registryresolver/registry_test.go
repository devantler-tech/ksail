...
func TestFormatRegistryURL(t *testing.T) {
	...
	for _, testCase := range getFormatRegistryURLTestCases() {
		t.Run(testCase.name, func(t *testing.T) {
			...
			result := registryresolver.FormatRegistryURL(
				testCase.host,
				testCase.port,
				testCase.repository,
			)
...
func TestDetectRegistryFromViper_NilViper(t *testing.T) {
	...
	info, err := registryresolver.DetectRegistryFromViper(nil)

	require.Error(t, err)
	require.ErrorIs(t, err, registryresolver.ErrViperNil)
	assert.Nil(t, info)
}
...
// TestRegistryInfo_Fields tests that Info struct fields work correctly.
func TestRegistryInfo_Fields(t *testing.T) {
	t.Parallel()

	info := registryresolver.Info{
		Host:       "localhost",
		Port:       5000,
...
func TestRegistryErrors(t *testing.T) {
	...
	t.Run("ErrNoRegistryFound", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registryresolver.ErrNoRegistryFound)
		assert.Contains(t, registryresolver.ErrNoRegistryFound.Error(), "unable to detect registry")
	})

	t.Run("ErrViperNil", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registryresolver.ErrViperNil)
		assert.Contains(t, registryresolver.ErrViperNil.Error(), "nil")
	})

	t.Run("ErrRegistryNotSet", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registryresolver.ErrRegistryNotSet)
		assert.Contains(t, registryresolver.ErrRegistryNotSet.Error(), "not set")
	})

	t.Run("ErrLocalRegistryNotConfigured", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registryresolver.ErrLocalRegistryNotConfigured)
		assert.Contains(t, registryresolver.ErrLocalRegistryNotConfigured.Error(), "not configured")
	})

	t.Run("ErrFluxNoSyncURL", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registryresolver.ErrFluxNoSyncURL)
		assert.Contains(t, registryresolver.ErrFluxNoSyncURL.Error(), "sync.url")
	})

	t.Run("ErrArgoCDNoRepoURL", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registryresolver.ErrArgoCDNoRepoURL)
		assert.Contains(t, registryresolver.ErrArgoCDNoRepoURL.Error(), "repoURL")
	})

	t.Run("ErrEmptyOCIURL", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registryresolver.ErrEmptyOCIURL)
		assert.Contains(t, registryresolver.ErrEmptyOCIURL.Error(), "empty")
	})
}
