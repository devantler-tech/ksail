...
> [!NOTE]
> `--config` is a **persistent flag** that must appear immediately after `ksail` and before any subcommand:
>
> ```bash
> ksail --config ksail.dev.yaml cluster create   ✅ correct
> ksail cluster --config ksail.dev.yaml create   ❌ incorrect
> ```
