...
func BenchmarkSanitizeYAMLOutput_SinglePod(b *testing.B) {
	...
	for b.Loop() {
		result, err := backup.SanitizeYAMLOutput(podYAML)
		if err != nil {
			b.Fatalf("sanitizeYAMLOutput: %v", err)
		}
...
func BenchmarkSanitizeYAMLOutput_PodList(b *testing.B) {
	...
	for b.Loop() {
		result, err := backup.SanitizeYAMLOutput(podListYAML)
		if err != nil {
			b.Fatalf("sanitizeYAMLOutput: %v", err)
		}
...
func BenchmarkCountYAMLDocuments_Single(b *testing.B) {
	...
	for b.Loop() {
		benchCountYAMLDocsSink = backup.CountYAMLDocuments(content)
	}
}
...
func BenchmarkFilterExcludedTypes_NoExclusions(b *testing.B) {
	types := []string{
		tTypeCRDs, tTypeNamespaces, "storageclasses",
		"persistentvolumes", "persistentvolumeclaims", "secrets",
		"configmaps", "serviceaccounts", "roles", "rolebindings",
		"clusterroles", "clusterrolebindings", tTypeServices,
		tTypeDeploys, "statefulsets", "daemonsets", "jobs",
		"cronjobs", "ingresses",
	}
	exclude := []string{}
	...
	for b.Loop() {
		benchFilterExcludedSink = backup.FilterExcludedTypes(types, exclude)
	}
}
...
func BenchmarkCreateTarball_Small(b *testing.B) {
	...
	err := backup.CreateTarball(srcDir, warmupOut, 6)
	if err != nil {
	...
	for b.Loop() {
		...
		err := backup.CreateTarball(srcDir, out, 6)
		if err != nil {
			b.Fatalf("createTarball: %v", err)
		}
	}
}
...
func setupBenchmarkFiles(b *testing.B, dir string, count, size int) {
	...
	for i := range count {
		subDir := filepath.Join(dir, "resources", tTypeField+string(rune('a'+i%26)))
		...
		err = os.WriteFile(
			filepath.Join(subDir, "resource.yaml"), payload, backup.FilePerm,
		)
		if err != nil {
