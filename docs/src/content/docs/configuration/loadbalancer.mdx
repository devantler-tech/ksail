...
1. **Talos on Docker**: LoadBalancer services are not currently supported. The `--load-balancer` flag is automatically set to `Disabled` for this combination, regardless of user input. MetalLB support for Talos on Docker is planned for a future release.
...
cloud-provider-kind will not be started (same as default behavior). LoadBalancer services will remain in `Pending` state.
