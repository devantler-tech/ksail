...
> **Note:** Release builds use `-ldflags="-s -w"` to strip debug symbols and reduce binary size by ~28% (302MB → 217MB). Development builds include debug symbols for better debugging experience.
...
- **CLI flags reference:** `docs/src/content/docs/cli-flags/` (contents are auto-generated by `.github/scripts/generate-cli-flags-docs.sh` from `ksail --help` output and should not be edited manually)
