...
2. **Merge triggers release:** Push to `main` builds binaries for all platforms, generates checksums, creates a GitHub Release, publishes the VSCode extension to the marketplace, and opens a PR to update the Homebrew cask in [`devantler-tech/homebrew-tap`](https://github.com/devantler-tech/homebrew-tap). Can also be triggered via `workflow_dispatch`.
