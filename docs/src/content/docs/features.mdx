...
- **Multi-distribution support** – [Vanilla, K3s, Talos, and VCluster](/concepts/#distributions) distributions
- **Native configuration** – Generates standard `kind.yaml`, `k3d.yaml`, Talos patch files, and `vcluster.yaml`
- **No vendor lock-in** – Use generated configs directly with Kind, K3d, Talos, or vCluster CLI
...
- **Resilient installations** – Automatic retry with exponential backoff for transient failures (HTTP 429 rate limits, network errors, timeouts)
...
**Distribution support:** Mirror registries are supported across all distributions:
- **Vanilla (Kind)**: Configured via containerd `hosts.toml` injection after cluster creation
- **K3s (K3d)**: Configured via K3d registries configuration before cluster creation
- **Talos**: Configured via machine config registry mirror patches
- **VCluster**: Configured via containerd `hosts.toml` injection into vCluster control plane and worker containers after cluster creation
...
Get interactive help with cluster configuration and troubleshooting using an AI-powered chat session. The assistant operates in three modes: **Agent** (full execution with approval), **Plan** (no execution, just guidance), and **Ask** (read-only investigation).
...
- **Three chat modes** – Agent (`</>`), Plan (`≡`), and Ask (`?`) modes for different workflows
- **Guided configuration** – Get recommendations for distributions, CNI, and other settings
- **Troubleshooting** – Diagnose cluster issues with read-only investigation (Ask mode)
- **Command execution** – Run KSail commands with your approval (Agent mode)
- **Step-by-step planning** – Learn workflows without making changes (Plan mode)
- **Context-aware** – Understands your ksail.yaml and project structure

**Mode Switching:** Press **Tab** to cycle between modes during a chat session.

**Prerequisites:** Requires [GitHub Copilot CLI](https://docs.github.com/en/copilot/concepts/agents/about-copilot-cli) installed and authenticated.
