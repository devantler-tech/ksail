...
See [Ephemeral Clusters (--ttl)](/features/ephemeral-clusters/) for the full TTL guide, or [PR Preview Clusters](/guides/pr-preview-clusters/) for the full action inputs reference and PR preview patterns.
...
- **Use `ksail workload validate`** with the target environment config before promoting manifests. See [Substitution Expansion (validate)](/configuration/workload-validate/) for how Flux `${VAR}` substitutions are handled during validation.
