...
- ¹⁰ AWS provider requires AWS credentials and an AWS account with EKS permissions — see [AWS Provider](/providers/aws/) for setup details; cluster lifecycle (create/delete/start/stop/list/info) and in-place managed node-group capacity updates are supported via eksctl, and EKS's managed CNI/CSI/LoadBalancer ship with the cluster. KSail-managed component installation is still in progress (the 🚧 cells below)
