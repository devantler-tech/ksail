...
The AWS provider will create managed [Amazon EKS](https://aws.amazon.com/eks/) clusters via [eksctl](https://eksctl.io/) once full provisioner support lands. Today, it is available for scaffolding configuration via `ksail cluster init` and is the only provider available for the [EKS distribution](/distributions/eks/).
