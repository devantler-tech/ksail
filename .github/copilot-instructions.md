...
- **MCP Command Tests**: Added 21 unit tests for `pkg/cli/cmd/mcp/` covering `NewMCPCmd` command structure, annotations, integration with root command, output buffer management, and nil safety (0% → 11.1% coverage); uses table-driven patterns and parallel execution; `runMCPServer` remains untested as it requires MCP server mocking (PR #2625)
