...
Ask the assistant natural-language questions like "why is my cluster broken?" and it will call the `cluster_read` tool with the `diagnose` subcommand, which surfaces failing pods and NotReady nodes via the Kubernetes API. Copilot then explains the root cause and suggests remediation.
...
The command is distribution-agnostic and works with Vanilla, K3s, Talos, and VCluster. Use `--format json` for machine-readable output in scripts or MCP tool calls. It exits 0 even when failures are reported; a non-zero exit code is reserved for unreachable clusters.
