...
> [!NOTE]
> Talos on Hetzner requires a Hetzner Cloud account and API token (`HCLOUD_TOKEN`). Talos on Omni requires a [Sidero Omni](https://www.siderolabs.com/platform/saas-for-kubernetes/) account, an `OMNI_SERVICE_ACCOUNT_KEY` environment variable, and `spec.cluster.omni.endpoint` configured in `ksail.yaml`. See the [Support Matrix](/support-matrix/) for more details.
