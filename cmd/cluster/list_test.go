...
func TestListCmd_NoAllFlag(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			factory := &fakeListFactory{clusters: tt.clusters}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			factory := &fakeListFactoryForAll{
				kindClusters: tt.kindClusters,
				k3dClusters:  tt.k3dClusters,
			}
			...
			output := buf.String()
			assert.Contains(t, output, tt.expectedKindSection)
			assert.Contains(t, output, tt.expectedK3dSection)
		})
	}
}
...
func setupTestConfig(t *testing.T, cmd *cobra.Command, allFlag bool) *ksailconfigmanager.ConfigManager {
	t.Helper()
...
func (f *fakeListFactory) Create( //nolint:ireturn // test double matches interface signature
	_ context.Context,
	_ *v1alpha1.Cluster,
...
func (f *fakeListFactoryForAll) Create( //nolint:ireturn // test double matches interface signature
	_ context.Context,
