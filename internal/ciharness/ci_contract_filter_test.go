...
//
// "Read" covers three ways the package depends on a file: every workflow, because
// TestNoDefaultBranchWorkflowCancelsRunsInProgress globs the whole workflows directory; every
// quoted workflow, action or script path in these test sources that exists on disk; and the
// test sources themselves, because editing one changes what the contract job asserts.
func TestWorkflowContractFilterCoversEveryFileTheHarnessReads(t *testing.T) {
	t.Parallel()

	patterns := workflowContractFilterPatterns(t)
	readPaths := harnessDependencyPaths(t)
	...
	assert.False(
		t,
		contractFilterMatches(t, withoutWorkflows, knownWorkflowPath),
		"negative control: without the workflows pattern the EKS workflow must be uncovered",
	)

	withoutHarness := slices.DeleteFunc(slices.Clone(patterns), func(pattern string) bool {
		return pattern == "internal/ciharness/**"
	})
	assert.False(
		t,
		contractFilterMatches(t, withoutHarness, knownSourcePath),
		"negative control: without the harness pattern this package's tests must be uncovered",
	)
}
...
// discovery source is guarded on its own first, so none can silently return nothing.
func harnessDependencyPaths(t *testing.T) []string {
	t.Helper()
...
func harnessSourcePaths(t *testing.T) []string {
	t.Helper()
	...
	for _, file := range files {
		paths = append(paths, "internal/ciharness/"+file)
	}
