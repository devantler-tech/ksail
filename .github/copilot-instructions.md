...
- **Visual Before/After Diff Table**: `ksail cluster update` now shows a formatted four-column table (Component, Before, After, Impact) after computing diffs; rows are sorted by severity — 🔴 recreate-required → 🟡 reboot-required → 🟢 in-place — and components with no change are omitted; column widths are computed dynamically; rendered by `displayChangesSummary` → `formatDiffTable` in `pkg/cli/cmd/cluster/update.go` (PR #2858)
