...
- VCluster runs as a virtual cluster inside a host cluster using the [Vind](https://github.com/loft-sh/vcluster) Docker driver
...
- **VCluster footnotes:**
  - ¹ CNI is managed by the host cluster — VCluster inherits networking from the host
  - ² CSI is managed by the host cluster — VCluster inherits storage from the host
  - ³ LoadBalancer is delegated to the host cluster
  - ⁴ Metrics Server is managed by the host cluster
  - ⁵ Mirror registries are not yet supported for VCluster (requires upstream Helm chart support)
...
- VCluster delegates CNI, CSI, metrics-server, and LoadBalancer to the host cluster — these are N/A for update operations
- VCluster does not support worker nodes (single-node virtual cluster)
