...
Automated AI-powered workflows run on schedules for continuous improvement: `daily-refactor`, `daily-code-health`, `daily-plan`, `daily-builder`, `daily-workflow-maintenance`, and `daily-docs`. These are Markdown-based agentic workflow definitions (not GitHub Actions YAML workflows) stored in `.github/workflows/*.md`.
