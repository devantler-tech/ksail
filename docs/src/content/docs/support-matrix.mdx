...
| Distribution     | Docker | Kubernetes | Hetzner | Omni | AWS   | GCP   | Azure |
| ---------------- | ------ | ---------- | ------- | ---- | ----- | ----- | ----- |
| Vanilla (Kind)   | ✅     | ✅         | ❌      | ❌   | ❌    | ❌    | ❌    |
| K3s (K3d)        | ✅     | ✅         | ❌      | ❌   | ❌    | ❌    | ❌    |
| Talos            | ✅     | ✅         | ✅      | ✅   | ❌    | ❌    | ❌    |
...
- Hetzner provider requires `HCLOUD_TOKEN` and a Talos ISO available in your Hetzner account (KSail tracks the compatible x86 default automatically; for ARM, look up the matching ISO ID in the Hetzner Cloud Console) — see [Hetzner Provider](/providers/hetzner/) for setup details
...
- ¹⁰ AWS provider requires AWS credentials and an AWS account with EKS permissions — see [AWS Provider](/providers/aws/) for setup details; cluster lifecycle (create/delete/start/stop/list/info) and in-place managed node-group capacity updates are supported via eksctl. EKS ships the Amazon VPC CNI and in-tree `Service` type `LoadBalancer` support with the cluster. The **Amazon EBS CSI Driver is an EKS add-on requested by KSail's default `eks.yaml` scaffold** (the `aws-ebs-csi-driver` addon) rather than being built into the distribution itself. The **AWS Load Balancer Controller is separate and experimental** — KSail's default path installs nothing, and it is opt-in via `spec.cluster.eks.experimentalAWSLoadBalancerController`. KSail-managed component installation is still in progress (the 🚧 cells below)
- ¹³ GCP provider requires Application Default Credentials plus `GOOGLE_CLOUD_PROJECT` and `GOOGLE_CLOUD_LOCATION` — see [GCP Provider](/providers/gcp/) for setup details; cluster lifecycle (create/delete/start/stop/list/info) is supported, and GKE's managed CNI/CSI/LoadBalancer/metrics ship with the cluster
- ¹⁴ Azure provider requires `DefaultAzureCredential`-resolvable credentials plus `AZURE_SUBSCRIPTION_ID` and `AZURE_RESOURCE_GROUP` — see [Azure Provider](/providers/azure/) for setup details; cluster lifecycle (create/delete/start/stop/list/info) is supported, and AKS's managed CNI/CSI/LoadBalancer/metrics ship with the cluster
...
- "Add-on" means the component is configured as an add-on in cluster configuration rather than bundled directly into the distribution
...
- **¹⁰ EKS**: The EKS cluster lifecycle (`create`/`delete`/`start`/`stop`/`list`/`info`) is supported via eksctl. `ksail cluster update` applies `desiredCapacity`, `minSize`, and `maxSize` changes to existing managed node groups in-place; instance-type changes and managed node-group additions/removals require recreation, while other managed-node-group fields are not currently diffed. Full KSail-managed component installer support (GitOps, security, and load-balancer add-ons beyond the VPC CNI that EKS ships by default and the EBS CSI driver scaffolded in `eks.yaml`) is still in progress; the AWS Load Balancer Controller can be installed behind the experimental `spec.cluster.eks.experimentalAWSLoadBalancerController` opt-in together with `loadBalancer: Enabled` (IAM permissions and subnet tags are prerequisites KSail does not create; clusters following AWS's IRSA setup can reuse their pre-created service account via `spec.cluster.eks.awsLoadBalancerControllerServiceAccount`). See [EKS Distribution](/distributions/eks/) for details.
...
See the generated [CLI Flags reference](/cli-flags/) for the complete, always-current list of command groups, commands, and flags.
...
| Workers                  | Recreate       | In-place  | In-place | N/A             | N/A            | In-place¹⁰ |
...
- Use `ksail cluster update --dry-run` to preview changes before applying; add `--output json` for machine-readable diff output (suitable for CI gating or [MCP tools](/integrations/mcp/))
