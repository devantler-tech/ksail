...
func TestFormatRegistryURL(t *testing.T) {
	...
	for _, testCase := range getFormatRegistryURLTestCases() {
		t.Run(testCase.name, func(t *testing.T) {
			...
			result := registry.FormatRegistryURL(testCase.host, testCase.port, testCase.repository)
...
func TestDetectRegistryFromViper_NilViper(t *testing.T) {
	...
	info, err := registry.DetectRegistryFromViper(nil)

	require.Error(t, err)
	require.ErrorIs(t, err, registry.ErrViperNil)
	assert.Nil(t, info)
}
...
func TestRegistryErrors(t *testing.T) {
	...
	t.Run("ErrNoRegistryFound", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registry.ErrNoRegistryFound)
		assert.Contains(t, registry.ErrNoRegistryFound.Error(), "unable to detect registry")
	})

	t.Run("ErrViperNil", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registry.ErrViperNil)
		assert.Contains(t, registry.ErrViperNil.Error(), "nil")
	})

	t.Run("ErrRegistryNotSet", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registry.ErrRegistryNotSet)
		assert.Contains(t, registry.ErrRegistryNotSet.Error(), "not set")
	})

	t.Run("ErrLocalRegistryNotConfigured", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registry.ErrLocalRegistryNotConfigured)
		assert.Contains(t, registry.ErrLocalRegistryNotConfigured.Error(), "not configured")
	})

	t.Run("ErrFluxNoSyncURL", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registry.ErrFluxNoSyncURL)
		assert.Contains(t, registry.ErrFluxNoSyncURL.Error(), "sync.url")
	})

	t.Run("ErrArgoCDNoRepoURL", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registry.ErrArgoCDNoRepoURL)
		assert.Contains(t, registry.ErrArgoCDNoRepoURL.Error(), "repoURL")
	})

	t.Run("ErrEmptyOCIURL", func(t *testing.T) {
		t.Parallel()
		require.Error(t, registry.ErrEmptyOCIURL)
		assert.Contains(t, registry.ErrEmptyOCIURL.Error(), "empty")
	})
}
