...
- **¹² KWOK CSI and cert-manager**: Local Path Provisioner (CSI) and cert-manager are not installed on KWOK. The Local Path Provisioner Deployment pod and cert-manager webhook pods require real container processes that KWOK does not provide, so readiness never becomes true and installation times out. KSail silently skips these components when `spec.cluster.distribution: KWOK` is set and emits a warning at cluster creation time.
