...
graph TB
    ...
    subgraph "Distributions"
        Provisioner[Provisioners<br/>Kind, K3d, Talos, VCluster]
    end
...
- **`schemas/`** — JSON schemas for validation and editor autocomplete

#### `pkg/cli/`
CLI wiring, commands, and terminal UI components.

- **`cmd/`** — Cobra command implementations
- **`ui/`** — Terminal UI components (chat TUI, ASCII art, confirmations)
...
- **`dockerutil/`** — Docker client lifecycle management
...
- **`provider/`** — Infrastructure providers
  - **`docker/`** — Docker-based clusters (Vanilla, K3s, Talos, VCluster)
  - **`hetzner/`** — Hetzner Cloud for Talos
  - **`omni/`** — Sidero Omni for Talos

##### Distribution Provisioning

- **`provisioner/cluster/`** — Distribution provisioners
  - **`kind/`** — Vanilla (standard Kubernetes via Kind SDK)
  - **`k3d/`** — K3s (lightweight K3s via K3d Cobra/SDK)
  - **`talos/`** — Talos (immutable Talos Linux via Talos SDK)
  - **`vcluster/`** — VCluster (virtual clusters via vCluster Go SDK with Vind Docker driver)
...
- **`installer/`** — Component installers
  ...
  - **`cloudproviderkind/`** — Cloud Provider KIND for LoadBalancer support (Vanilla)
  - **`gatekeeper/`** — Gatekeeper policy engine
  - **`hcloudccm/`** — Hetzner Cloud Controller Manager
  - **`hetznercsi/`** — Hetzner CSI driver
  - **`kyverno/`** — Kyverno policy engine
  - **`localpathcsi/`** — Local path CSI
  - **`longhorncsi/`** — Longhorn CSI
  - **`metallb/`** — MetalLB LoadBalancer (Talos)
  - **`metricsserver/`** — Metrics Server
  - **`internal/hetzner/`** — Shared utilities for Hetzner installers
...
Talos demonstrates the provider/provisioner separation:

1. **Talos Provisioner** (`pkg/svc/provisioner/cluster/talos/`)
   - Generates Talos machine configs and patches
   - Bootstraps Talos Kubernetes
   - Manages cluster lifecycle

2. **Docker Provider** (`pkg/svc/provider/docker/`)
   - Runs Talos nodes as Docker containers
   - Used for local development

3. **Hetzner Provider** (`pkg/svc/provider/hetzner/`)
   - Runs Talos nodes on Hetzner Cloud servers
   - Installs Hetzner CCM and CSI drivers

4. **Omni Provider** (`pkg/svc/provider/omni/`)
   - Manages Talos clusters via Sidero Omni API
   - Provides UI and API management

The **same Talos provisioner** works with all three providers, enabling consistent Talos experience across environments.
...
**Implementation:** `pkg/svc/chat/`

Uses GitHub Copilot SDK to provide interactive cluster configuration and troubleshooting:

- **Three modes:**
  - **Agent Mode** (`</>`) — Full tool execution with permission prompts
  - **Plan Mode** (`≡`) — No execution; AI describes steps
  - **Ask Mode** (`?`) — Read-only investigation; write tools blocked

- **Features:**
  - Tab to cycle between modes
  - `^O` for model picker (lazy-loaded)
  - `^E` for reasoning effort (low, medium, high)
  - `^H` for session history
  - Auto model selection with 10% discount
  - Quota tracking for premium requests
  - Infinite sessions with background compaction

- **Authentication:**
  - Supports `KSAIL_COPILOT_TOKEN` and `COPILOT_TOKEN`
  - Filters `GITHUB_TOKEN`/`GH_TOKEN` to avoid scope issues

### MCP Server

**Command:** `ksail mcp`

**Implementation:** `pkg/svc/mcp/`

Exposes KSail as a Model Context Protocol server for Claude and other AI assistants:

- Tools for cluster management (create, update, delete, info)
- Workload deployment (apply, reconcile)
- Configuration helpers
- Runs as background server that AI assistants connect to
...
- **Coverage target:** Individual package coverage varies; overall project aim is high coverage
- **Mocking:** Uses testify/mock for interfaces
- **Export pattern:** `export_test.go` files expose unexported symbols for testing without leaking to production
- **Parallel execution:** Tests use `t.Parallel()` when safe
- **Static error sentinels:** Tests use static errors (e.g., `errFake`) for predictable mocking

**Example:** `pkg/client/helm/operations_test.go` tests Helm operations with mocked interfaces.

### Integration Tests

Test real cluster operations with actual tools:
- Create real Kind/K3d/VCluster clusters
- Deploy real workloads
- Verify end-to-end workflows

### System Tests (CI)

Comprehensive CI tests cover:
- All distribution × provider combinations
- All component combinations (CNI, CSI, GitOps, etc.)
...
- **Daily Test Improver** — Adds unit tests to increase coverage
- **Daily Perf Improver** — Adds benchmarks and performance improvements
- **Daily Refactor** — Refactors code for maintainability
- **Daily Progress** — Implements roadmap features
- **Daily Backlog Burner** — Tackles backlog issues

**Location:** `.github/workflows/*.md` (agentic workflow definitions)
...
1. **Tag creation** triggers CI workflow
2. **GoReleaser** builds binaries for all platforms/architectures:
   - Linux (amd64, arm64)
   - macOS (arm64)
   - Windows (amd64, arm64)
3. **Artifacts uploaded** to GitHub Releases
4. **Checksums generated** for verification
5. **VSCode extension** packaged and published to marketplace
