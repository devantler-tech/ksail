...
func TestMirrorSpec_ResolveCredentials_EnvVars(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			spec: registry.MirrorSpec{ // #nosec G101 -- test data using env var placeholders
				Host:     "ghcr.io",
				Remote:   "https://ghcr.io",
				Username: "${TEST_USER}",
				Password: "${TEST_TOKEN}",
		...
		},
		{
			...
			spec: registry.MirrorSpec{ // #nosec G101 -- test data with env var placeholder
				Host:     "quay.io",
				Remote:   "https://quay.io",
...
func TestParseMirrorSpecs_AtSignInURL(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expected: []registry.MirrorSpec{
				{ // #nosec G101 -- test data with URL containing credentials
					Host:     "docker.io",
