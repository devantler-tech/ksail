...
func ensureRepositoryConfig(settings *helmv4cli.EnvSettings) (string, error) {
	...
	mkdirErr := os.MkdirAll(repoDir, repoDirMode) // #nosec G703 -- path from Helm settings
	if mkdirErr != nil {
...
func ensureRepositoryCache(settings *helmv4cli.EnvSettings) (string, error) {
	...
	mkdirCacheErr := os.MkdirAll(repoCache, repoDirMode) // #nosec G703 -- path from Helm settings
