...
KSail automatically approves Certificate Signing Requests (CSRs) for kubelet serving certificates when metrics-server is enabled. When `serverTLSBootstrap: true` is active, kubelets request proper TLS certificates via CSR instead of self-signed certificates, enabling secure TLS communication with metrics-server. KSail uses different implementations per distribution:

- **Vanilla**: installs [postfinance/kubelet-csr-approver](https://github.com/postfinance/kubelet-csr-approver) via Helm
- **Talos**: embeds [alex1989hu/kubelet-serving-cert-approver](https://github.com/alex1989hu/kubelet-serving-cert-approver) via `cluster.inlineManifests` during bootstrap
