...
- **¹⁰ EKS**: The EKS cluster lifecycle (`create`/`delete`/`start`/`stop`/`list`/`info`) is supported via eksctl. Full KSail-managed component installer support (GitOps, security, and load-balancer add-ons beyond eksctl's built-in VPC CNI / EBS CSI) and `ksail cluster update` for EKS are still in progress. See [EKS Distribution](/distributions/eks/) for details.
