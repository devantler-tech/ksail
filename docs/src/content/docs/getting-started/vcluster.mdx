...
VCluster runs a full Kubernetes control plane (API server, scheduler, controller manager, etcd) in a single Docker container. Worker nodes are optional additional containers. Pods are scheduled to worker nodes or run directly in the control plane container. When using host-cluster deployment mode, selected resources can be synced between the virtual and host cluster.
