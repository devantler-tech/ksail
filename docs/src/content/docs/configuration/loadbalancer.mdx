...
Talos on Hetzner Cloud uses the [Hetzner Cloud Controller Manager](https://github.com/hetznercloud/hcloud-cloud-controller-manager) to provision real cloud load balancers. LoadBalancer is **enabled by default** — KSail automatically installs hcloud-ccm when `loadBalancer` is `Default` or `Enabled`.
