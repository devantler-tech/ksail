...
- **a** — Allow always (switches to Autopilot mode, auto-approving all future operations in this session)
- **n / Esc** — Deny
...
Use **Plan mode** (`≡`) to learn workflows and review potential changes, **Interactive mode** (`</>`) to execute operations with approval, and **Autopilot mode** (`⚡`) for trusted workflows that should run unattended. Press **Tab** to switch modes as needed.
...
Start with Plan mode for learning and reviewing workflows, then switch to Interactive mode for execution with approval or Autopilot mode for trusted unattended operations. Adjust reasoning effort (`Ctrl+E`) based on task complexity, use `--model auto` to optimize costs, and filter session history (`Ctrl+H` then `/`) to find past conversations.
