...
Runs Kubernetes nodes as Docker containers locally. Default provider for all distributions, requires only Docker. **Supported distributions:** Vanilla, K3s, Talos, VCluster. See [Docker Provider](/providers/docker/), [Docker docs](https://docs.docker.com/), and [Docker Desktop](https://www.docker.com/products/docker-desktop/).
...
[Cilium](https://cilium.io/) is an eBPF-based CNI offering networking, security, and observability with features like transparent encryption and service mesh. KSail enables Cilium's [Gateway API](https://gateway-api.sigs.k8s.io/) integration by default (`gatewayAPI.enabled: true`) and automatically pre-installs the standard [Gateway API CRDs](https://gateway-api.sigs.k8s.io/guides/) as a prerequisite. On Docker-based clusters **without** a LoadBalancer, host network mode (`gatewayAPI.hostNetwork.enabled: true`) is configured so Gateway API routes traffic via the Docker bridge using port mappings. On Docker-based clusters where a LoadBalancer integration is enabled (for example, Cloud Provider KIND for Vanilla-on-Docker or MetalLB), host network mode is skipped and traffic flows via LoadBalancer-assigned external IPs instead. See [documentation](https://docs.cilium.io/), [concepts](https://docs.cilium.io/en/stable/overview/intro/), [Gateway API guide](https://docs.cilium.io/en/stable/network/servicemesh/gateway-api/gateway-api/), [network policies](https://docs.cilium.io/en/stable/security/policy/), and [Gateway API with KSail](/guides/gateway-api/).
