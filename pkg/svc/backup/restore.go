...
// Validate checks the configured existing-resource-policy, returning
// [ErrInvalidResourcePolicy] for unsupported values.
func (r *Restorer) Validate() error {
	if r.opts.ExistingResourcePolicy != PolicyNone &&
		r.opts.ExistingResourcePolicy != PolicyUpdate {
...
// Restore validates options, extracts the configured backup archive, and
// applies its resources to the target cluster, writing human-readable progress
// to writer. It returns the parsed [BackupMetadata] so callers can display
// archive details. This is a convenience wrapper over Validate/Extract/
// RestoreExtracted for callers that do not need to interleave metadata display.
func (r *Restorer) Restore(ctx context.Context, writer io.Writer) (*BackupMetadata, error) {
	err := r.Validate()
	if err != nil {
		return nil, err
	}

	archive, err := r.Extract()
	if err != nil {
		return nil, err
	}
	...
	err = r.RestoreExtracted(ctx, archive, writer)
	if err != nil {
