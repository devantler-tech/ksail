...
Exposes KSail as a Model Context Protocol server for Claude and other AI assistants. Tools are **auto-generated** from the Cobra command tree by `pkg/toolgen/` — the same mechanism used for the chat assistant. All CLI commands (both read and write operations) are automatically exposed; no manual tool registration is needed.

Commands are consolidated by permission annotation (`ai.toolgen.permission`) into 6 tools: `cluster_read`, `cluster_write`, `workload_read`, `workload_write`, `cipher_read`, and `cipher_write`. Adding a new CLI command with the appropriate annotation automatically makes it available as an MCP tool. See [MCP](/mcp/) for setup instructions.
