...
Kubernetes distributions package core components with additional tooling for specific use cases. KSail supports six distributions: Vanilla, K3s, Talos, VCluster, KWOK, and EKS. Local distributions can run on Docker or on the Kubernetes provider; Talos can also run on Hetzner Cloud and Sidero Omni; EKS runs on AWS.
...
[Amazon EKS](https://aws.amazon.com/eks/) is AWS's managed Kubernetes service. KSail uses [eksctl](https://eksctl.io/) to scaffold a declarative `eks.yaml` (`ClusterConfig`). `ksail cluster init` is available; full cluster creation support is planned. Requires AWS credentials and an account with EKS permissions. See [EKS Distribution](/distributions/eks/) and [AWS Provider](/providers/aws/).
...
Runs nested cluster nodes as pods inside an existing host Kubernetes cluster. The nested API server is exposed through a stable, server-side endpoint (Gateway API, LoadBalancer, or NodePort) so `kubectl` keeps working after `ksail` exits — no port-forward required. **Supported distributions:** Vanilla, K3s, Talos, VCluster, KWOK. **Requirements:** a host Kubernetes cluster that allows privileged pods and a routable exposure address. See [Kubernetes Provider](/providers/kubernetes/).
