...
Changes are classified the same way as `cluster update`: in-place, reboot-required, or recreate-required. Unlike `cluster update --dry-run`, `cluster diff` is a pure read operation — it never applies or stages any changes.
