...
````go
import "github.com/devantler-tech/ksail/v5/pkg/fsutil"
...
When canonicalizing an **output** path that may not yet exist (e.g., `--output`), create parent directories with `os.MkdirAll` first, then call `EvalCanonicalPath`. For safe path-constrained reads, use `fsutil.ReadFileSafe` instead of reimplementing containment checks.
