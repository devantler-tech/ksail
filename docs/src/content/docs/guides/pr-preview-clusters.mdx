...
| `init` | `true` | Run `ksail project init` before create |
...
- **Pin the action version** using a release tag (e.g. `@v7.56.0`) or a commit SHA for reproducible CI, or `@main` for bleeding-edge — there is currently no floating major tag like `@v7`.
...
- [Testing in CI/CD](/guides/cicd-integration/) — broader CI testing patterns with KSail
