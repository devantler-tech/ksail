...
type OptionsHetzner struct {
	...
	// the first control-plane server; node-side ownership handover (a Talos VIP block that
	// makes the elected leader answer on the address) is not yet generated by KSail and must
	// be provided as a Talos machine-config patch until then. Defaults to false (no floating
	// IP; rendered configs are unchanged).
	FloatingIPEnabled bool `json:"floatingIPEnabled,omitzero" jsonschema_description:"Provision a Hetzner floating IP and render it as the stable Kubernetes/Talos API endpoint (endpoint + certificate SANs). Defaults to false. Node-side ownership handover (Talos VIP) must be provided as a machine-config patch until KSail generates it."` //nolint:lll,tagliatelle // floatingIP casing matches the sibling *PublicIPv4 fields
