...
- ¹⁰ AWS provider requires AWS credentials and an AWS account with EKS permissions — see [AWS Provider](/providers/aws/) for setup details; cluster lifecycle (create/delete/start/stop/list/info) and in-place managed node-group capacity updates are supported via eksctl. EKS ships the Amazon VPC CNI and in-tree `Service` type `LoadBalancer` support with the cluster. The **Amazon EBS CSI Driver is not installed by default** — it is a separate EKS add-on, so block-volume provisioning is unavailable until you add it. The **AWS Load Balancer Controller is separate and experimental** — KSail's default path installs nothing, and it is opt-in via `spec.cluster.eks.experimentalAWSLoadBalancerController`. KSail-managed component installation is still in progress (the 🚧 cells below)
