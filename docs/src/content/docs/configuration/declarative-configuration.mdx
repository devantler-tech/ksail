...
Use `${VARIABLE_NAME}` to reference environment variables. If a variable is not set, it expands to an empty string.
...
spec:
  ...
  cluster:
    ...
    distributionConfig: "${CONFIG_DIR}/kind.yaml"
    localRegistry:
      registry: "${REGISTRY_USER}:${REGISTRY_PASS}@ghcr.io"
    vanilla:
      mirrorsDir: "${MIRRORS_DIR}"
    talos:
      config: "${TALOS_CONFIG_PATH}"
    hetzner:
      sshKeyName: "${HCLOUD_SSH_KEY}"
  workload:
    sourceDirectory: "${WORKLOAD_DIR}"
  chat:
    model: "${CHAT_MODEL}"
```
...
spec:
  cluster:
    ...
    distributionConfig: "${ENV}/kind.yaml"
  workload:
    sourceDirectory: "${ENV}/k8s"
```
