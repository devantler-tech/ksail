...
spec:
  ...
  provider:
    hetzner:
      ...
      # Restrict Kubernetes API (6443) and Talos API (50000) access to specific CIDRs.
      # When empty, both APIs are open to 0.0.0.0/0 and ::/0.
...
| `hetzner.allowedCidrs` | list | _(empty)_ | CIDR blocks allowed to access the Kubernetes API (6443) and Talos API (50000) on control-plane nodes. Applied to both the Hetzner Cloud Firewall rules and the Talos OS-level ingress firewall. When empty, both APIs are open to `0.0.0.0/0` and `::/0`. |
