...
| `hetzner.serverLimit` | int | `10` | Maximum total Hetzner servers for this cluster. Validation checks `control-planes + workers + effective autoscaler capacity`, where effective autoscaler capacity is `sum(pool.max)` or, when `autoscaler.node.maxNodesTotal > 0`, `min(sum(pool.max), autoscaler.node.maxNodesTotal)`. KSail rejects autoscaler configurations that exceed this limit. Set to `0` to use the default of 10. |
...
| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `autoscaler.node.enabled` | string | `Disabled` | `Enabled` — defer node scaling to an external autoscaler; `Disabled` — KSail manages node counts directly |
...
| `autoscaler.node.scaleDownUnneededTime` | string | _(empty)_ | How long a node must be unneeded before it is eligible for scale-down (e.g. `10m`) |
...
KSail supports the [Kubernetes Cluster Autoscaler](https://github.com/kubernetes/autoscaler) for Hetzner Talos clusters. When enabled, the autoscaler dynamically adds and removes worker nodes based on workload demand.

Configure node pools and enable the autoscaler in `ksail.yaml`:
...
spec:
  cluster:
    autoscaler:
      node:
        enabled: Enabled
        expander: LeastWaste           # LeastWaste, MostPods, Random, Price
        maxNodesTotal: 20              # hard cap across all pools
        scaleDownUnneededTime: "10m"
        pools:
          - name: workers-fsn1
            serverType: cx23
  ...
  provider:
    hetzner:
      serverLimit: 20   # must be ≥ controlPlanes + workers + sum(pool.Max) across all pools
```

| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `autoscaler.node.enabled` | enum | `Disabled` | Enable node autoscaling (`Enabled` or `Disabled`) |
| `autoscaler.node.expander` | enum | `LeastWaste` | Node selection strategy (`LeastWaste`, `MostPods`, `Random`, `Price`) |
| `autoscaler.node.maxNodesTotal` | int | `0` (unlimited) | Hard cap on total nodes across all pools. When > 0, acts as a ceiling even if pool maximums sum higher. |
| `autoscaler.node.scaleDownUnneededTime` | string | – | Duration a node must be unneeded before scale-down (e.g. `10m`) |
| `autoscaler.node.pools[].name` | string | – | Unique pool identifier (letters, digits, hyphens; max 63 chars) |
| `autoscaler.node.pools[].serverType` | string | – | Hetzner server type (e.g. `cx23`, `cax11`) |
| `autoscaler.node.pools[].location` | string | – | Hetzner datacenter (e.g. `fsn1`, `nbg1`) |
| `autoscaler.node.pools[].min` | int | – | Minimum nodes in the pool |
| `autoscaler.node.pools[].max` | int | – | Maximum nodes in the pool |

> [!NOTE]
> `hetzner.serverLimit` (default `10`) is a safety cap on the total number of Hetzner servers KSail is allowed to provision. When using the autoscaler, set `serverLimit` to at least `controlPlanes + workers + sum(pool.Max)` to avoid validation errors.

> [!NOTE]
> Pod autoscaler fields (`autoscaler.pod.horizontal` and `autoscaler.pod.vertical`) are reserved for future use and have no effect in the current release.
...
**`autoscaler pool capacity exceeds serverLimit`** — The sum of your autoscaler pool maximums (plus `controlPlanes` + `workers`) exceeds `hetzner.serverLimit`. Increase `serverLimit` or reduce your pool `max` values.
