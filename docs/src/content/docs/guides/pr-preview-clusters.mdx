...
- **`--ttl` flag** (via `args`) — safety-net auto-teardown if the cleanup step is skipped (e.g. runner crash)
    ...
    steps:
      ...
      - name: Provision preview cluster with GitOps
        ...
        with:
          ...
          args: "--ttl 1h"        # safety-net auto-teardown if cleanup is missed
...
The `--ttl` flag makes `ksail cluster create` auto-destroy the cluster after a duration — even if the GitHub Actions runner crashes or the `delete` step is skipped. Pass it via the `args` input:

```yaml
- name: Provision preview cluster with TTL safety net
  id: cluster
  uses: devantler-tech/ksail/.github/actions/ksail-cluster@main
  with:
    args: "--ttl 1h"       # auto-destroy after 1 hour if cleanup is missed
    validate: "true"
    push: "true"
    reconcile: "true"
    delete: "true"         # primary cleanup — still runs on success and failure
```

> [!TIP]
> `delete: true` is the primary cleanup mechanism — it runs immediately when the job finishes (including on failure). `--ttl` is a best-effort safety net: the cluster auto-destroys when the TTL elapses, **provided the `ksail cluster create` process stays alive**. Use both together for defence in depth. See [Ephemeral Clusters (`--ttl`)](/guides/ephemeral-clusters/) for full TTL details.
...
| `args` | `""` | Extra args passed to `cluster init` |
