...
- **MetalLB Installer Tests**: Added comprehensive unit tests for `pkg/svc/installer/metallb/` covering CRD waiting, IP address pool creation, and L2 advertisement configuration (6.6% → 23.0% coverage); follows the `export_test.go` pattern (exports `ExportWaitForCRDsWithOptions`, `ExportEnsureIPAddressPool`, `ExportEnsureL2Advertisement`); uses `fake.NewSimpleDynamicClient()` for CRD/resource assertions, `helm.NewMockInterface(t)` for Helm install/uninstall, `t.Parallel()`, and `require.NoError/Error`; tests document limitations with Server-Side Apply in the fake client (PR #2673)
