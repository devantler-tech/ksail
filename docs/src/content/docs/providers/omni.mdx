...
KSail detects the Omni provider automatically from the server URL (`*.omni.siderolabs.io`) and embeds the correct distribution and provider metadata in the backup archive. This works even when the Omni-generated context name (e.g., `devantler-prod`) doesn't follow KSail's default naming conventions. See [Backup & Restore](/features/#backup--restore) for full details.

### Kubeconfig Tokens

KSail automatically refreshes your Omni kubeconfig token before it expires. Tokens have a **30-day TTL** and are renewed transparently before any CLI command runs (when the token has 5 minutes or less remaining). No manual action is required.

> [!NOTE]
> Auto-refresh is skipped when you pass `--kubeconfig` explicitly. In that case, you are managing the kubeconfig yourself and KSail will not modify it.
...
**`MachineClass and Machines are mutually exclusive (set one or the other)`** — Both `spec.cluster.omni.machineClass` and `spec.cluster.omni.machines` are set in `ksail.yaml`. Remove one: use `machineClass` for dynamic allocation, `machines` for static allocation, or omit both for auto-discovery.
