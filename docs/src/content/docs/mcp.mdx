...
| Tool | Subcommands |
|------|-------------|
| `cluster_read` | `info`, `list`, `connect`, `switch` (pass cluster name via `args`; mutates local kubeconfig current-context), `diagnose` (pass `["--output", "json"]` via `args` for structured JSON output; the tool returns the JSON as a string that the client must parse) |
...
For step-by-step setup instructions for each AI client — including PATH configuration, example prompts, and a tool reference — see [Using KSail with AI Assistants](/guides/ai-mcp/).
