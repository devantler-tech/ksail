...
`spec.cluster.talos.extensions` builds an [Image Factory](https://factory.talos.dev/) schematic so system extensions (for example `iscsi-tools` for storage, or `qemu-guest-agent` on cloud providers) are baked into the boot image. Before a new Hetzner snapshot or a Talos upgrade uses that schematic, KSail registers it with Image Factory and checks it is stored under the same ID, so a new combination of extensions and kernel arguments works the first time it is used. On Hetzner, a schematic snapshot requires a Talos version pin. If you boot a custom Hetzner ISO instead, pin `spec.cluster.talos.version` to the release that ISO boots; KSail cannot infer it from the ISO ID. See the [Hetzner Provider](/providers/hetzner/#using-a-custom-talos-iso) page for configuration and the [CLI flag reference](/cli-flags/cluster/cluster-root/) for every init/create/update flag.
