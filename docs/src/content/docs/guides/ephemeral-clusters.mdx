...
Use the `ksail-cluster` GitHub Actions composite action to set up KSail, cache Helm charts and mirror images, and run `ksail project init` + `ksail cluster create` in a single step. See [PR Preview Clusters](/guides/pr-preview-clusters/) for the full action reference and GitOps CI workflows, or [Testing in CI/CD](/guides/cicd-integration/) for integration test patterns.
