...
| New Provisioner | Implement the `Provisioner` interface; add a distribution guide under `docs/distributions/` |
| New Installer | Implement the `Installer` interface; add configuration fields to the API types |

For a new Provisioner, also add a distribution guide in `docs/distributions/`. For a new Installer, also add configuration fields to the API types and update the JSON schema.
