...
3. **Talos ISO or Schematic** *(Talos distribution only)* — Either a Talos Linux ISO must be available in your Hetzner Cloud project (x86 default: `125127` for Talos 1.12.4; for ARM, look up the matching ISO ID in the Hetzner Cloud Console), or a [Talos factory schematic ID](https://factory.talos.dev) can be provided via `spec.cluster.talos.schematicId` so KSail builds the snapshot automatically. See [Talos options](/configuration/declarative-configuration/#distribution-and-tool-options).
