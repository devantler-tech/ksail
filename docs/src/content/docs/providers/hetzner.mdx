...
| `hetzner.serverLimit` | int | `10` | Maximum total Hetzner servers for this cluster (control-planes + workers + autoscaler pool capacity). KSail rejects autoscaler configurations that exceed this limit. Set to `0` to use the default of 10. |
      ...
      node:
        ...
        pools:
          ...
          - name: workers-nbg1
            ...
            max: 3
      pod:
        horizontal: Disabled       # Enabled | Disabled (installs metrics-server)
        vertical: Disabled         # Enabled | Disabled (installs VPA)
  provider:
    hetzner:
      serverLimit: 20              # Must be ≥ controlPlanes + workers + sum(pool.max)
```
...
| `autoscaler.pod.horizontal` | string | `Disabled` | `Enabled` installs [metrics-server](https://github.com/kubernetes-sigs/metrics-server) and activates HPA support |
| `autoscaler.pod.vertical` | string | `Disabled` | `Enabled` installs the [Vertical Pod Autoscaler](https://github.com/kubernetes/autoscaler/tree/master/vertical-pod-autoscaler) |

> [!NOTE]
> `spec.provider.hetzner.serverLimit` guards against exceeding your Hetzner Cloud project quota. KSail rejects configurations where `controlPlanes + workers + sum(pool.max)` exceeds `serverLimit`. Raise this limit to match your actual Hetzner project quota.
