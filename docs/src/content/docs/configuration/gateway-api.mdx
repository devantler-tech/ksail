...
> [!NOTE]
> This guide assumes you are using **Cilium** as your CNI. Gateway API is not automatically enabled on the Default (kindnet) CNI. If you initialized your cluster without specifying `--cni`, run `ksail project init --cni Cilium` to get a Cilium-enabled cluster.
...
If `ACCEPTED` is `False`, wait a few seconds for Cilium to finish initializing. You can also check:
...
Replace each `Ingress` with a `Gateway` (one per entry point) and one or more `HTTPRoute` resources. See [Expose a Service with HTTPRoute](#expose-a-service-with-httproute) for a complete example. For a detailed migration walkthrough, see the [Kubernetes Gateway API migration guide](https://gateway-api.sigs.k8s.io/guides/migrating-from-ingress/).
