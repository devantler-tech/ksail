...
## CI/CD with the Reusable Workflow

KSail publishes a reusable GitHub Actions workflow that provisions a cluster in CI with a single `uses:` line. This is the fastest way to add ephemeral cluster smoke tests to any repository.
...
jobs:
  cluster:
    uses: devantler-tech/ksail/.github/workflows/ksail-cluster.yml@main
    with:
      distribution: K3s     # Vanilla (default), K3s, Talos, or VCluster
      ttl: "30m"             # cluster TTL (default: 1h)
      ksail-version: latest  # KSail release version (default: latest)
```

The workflow provisions a cluster and exports the kubeconfig path as the `kubeconfig` output.
...
| `ttl` | string | `1h` | Cluster TTL; passed to `--ttl` |
...
> [!NOTE]
> The workflow runs on `ubuntu-latest` with Docker pre-installed. The KSail binary is downloaded from [GitHub Releases](https://github.com/devantler-tech/ksail/releases). The cluster runs inside Docker containers on the runner and is cleaned up automatically when the runner is recycled.
