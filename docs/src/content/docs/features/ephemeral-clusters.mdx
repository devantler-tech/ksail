...
KSail writes the kubeconfig context to the configured kubeconfig path (default `~/.kube/config`, configurable via `spec.cluster.connection.kubeconfig`). Use `kubectl`, `k9s`, or `ksail cluster connect` immediately after creation. For context name formats per distribution, see [Companion Tools](/integrations/companion-tools/).
...
Use the `ksail-cluster` GitHub Actions composite action to set up KSail, cache Helm charts and mirror images, and run `ksail cluster init` + `ksail cluster create` in a single step. See [PR Preview Clusters](/guides/pr-preview-clusters/) for the full action reference and GitOps CI workflows, or [Testing in CI/CD](/use-cases/#testing-in-cicd) for integration test patterns.
