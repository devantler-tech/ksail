...
For multi-environment patterns (separate config files per environment, environment variable expansion, and CI/CD usage), see [Multi-Environment Workflows](/guides/multi-environment/).
...
Values in `ksail.yaml` support `${VAR}` expansion from the environment at runtime. This is distinct from the `KSAIL_`-prefixed overrides above: an override replaces a whole field, while `${VAR}` substitutes an environment value _into_ a field. It lets you keep one config in version control while values that vary per environment—or that are secret—come from the environment when KSail runs.
...
This is how a single `ksail.yaml` serves many environments and stays credential-free in version control—see [Multi-Environment Workflows](/guides/multi-environment/) and the [Reference Architecture](/concepts/reference-architecture/).

:::note
This expansion applies to `ksail.yaml` values. Flux performs its own separate `${var}` post-build substitution _inside your manifests_; that is a different mechanism, covered in [Validate Workloads](/configuration/workload-validate/).
:::
...
- **[Gateway API with Cilium](/configuration/gateway-api/)** – Expose services with `HTTPRoute` (auto-enabled when `cni: Cilium`)
