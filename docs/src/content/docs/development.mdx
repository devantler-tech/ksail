...
| New Installer | Implement the `Installer` interface; add configuration fields to the API types and update the JSON schema |
...
Five AI-powered workflows run on schedules or triggers (manageable via `gh aw`): `repo-assist`, `daily-workflow-maintenance`, `daily-docs`, `weekly-strategy`, and `ci-doctor`.
...
3. **Tag-triggered CD:** Any push of a version tag triggers `.github/workflows/cd.yaml`, which builds and publishes binaries, creates the GitHub Release, deploys documentation, publishes the VSCode extension, and opens a Homebrew tap PR.
