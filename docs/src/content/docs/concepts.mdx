...
Kubernetes distributions package core components with additional tooling for specific use cases. KSail supports six distributions: Vanilla, K3s, Talos, VCluster, KWOK, and EKS. All local distributions run on Docker; Talos can also run on Hetzner Cloud and Sidero Omni; EKS runs on AWS.
