...
- **CSI (Local Path Provisioner)** — the local-path-provisioner pods and helpers require real container execution; on KWOK they never become truly ready and installation waits time out.
- **cert-manager** — cert-manager webhook pods must serve real TLS endpoints; they time out on KWOK.
- **Flux** — the flux-operator pod is simulated but never registers Flux CRDs; use ArgoCD instead.
