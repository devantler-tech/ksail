...
For general KSail YAML options, see the [Configuration Reference](/configuration/). KSail scaffolds a `kwok/` kustomize directory that KWOK's config loader assembles automatically.
...
├── simulation.yaml          # Cluster-level simulation CRDs
├── node-not-ready.yaml      # Node NotReady chaos stage (opt-in)
└── pod-failure.yaml         # Pod failure chaos stage (opt-in)
````
...
The `kwok/simulation.yaml` scaffolded by KSail adds four Cluster-level CRDs not provided by default:
