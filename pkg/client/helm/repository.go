...
func ensureRepositoryConfig(settings *helmv4cli.EnvSettings) (string, error) {
	...
	mkdirErr := os.MkdirAll(repoDir, repoDirMode) //nolint:gosec // path from Helm config
	if mkdirErr != nil {
...
func ensureRepositoryCache(settings *helmv4cli.EnvSettings) (string, error) {
	...
	mkdirCacheErr := os.MkdirAll(repoCache, repoDirMode) //nolint:gosec // Helm cache path
