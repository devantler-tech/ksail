...
An explicit `--name` or a loaded project configuration takes precedence over context-based name detection. When the name comes from the selected EKS context, its region is the fallback after the configured AWS region environment variable and project configuration. Other contexts for the same name do not override that selected region. An explicit name still requires an unambiguous region; it does not inherit an unrelated selected context's region.
