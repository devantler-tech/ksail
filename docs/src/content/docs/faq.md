...
KSail currently supports **Vanilla** (Kind), **K3s** (K3d), **Talos**, **VCluster** (Vind), **KWOK** (kwokctl, simulated), and **EKS** (scaffolding available; `cluster create` is not yet functional). See the [Support Matrix](/support-matrix/) for current provider compatibility and feature status.
