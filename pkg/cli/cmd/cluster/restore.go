...
func extractTarEntries(tarReader *tar.Reader, destDir string) error {
	for {
		...
		if header.Typeflag == tar.TypeDir {
			err = os.MkdirAll(targetPath, dirPerm) //nolint:gosec // G703: sanitized path
			if err != nil {
		...
		err = os.MkdirAll(filepath.Dir(targetPath), dirPerm) //nolint:gosec // G703: sanitized path
		if err != nil {
			return fmt.Errorf(
...
func injectRestoreLabels(
	...
	if err != nil {
		_ = os.Remove(tmpFile.Name()) //nolint:gosec // G703: path is temp file
