...
```bash
ksail project init --name my-cluster --distribution K3s --control-planes 1 --workers 2
ksail cluster create
ksail cluster info && kubectl get nodes  # verify
ksail cluster delete                     # cleanup
```

`ksail project init` creates `ksail.yaml` (KSail config) and `k3d.yaml` (K3d cluster config).
...
```bash
ksail project init --name my-cluster --distribution K3s \
...
ksail project init --name my-cluster-2 --distribution K3s
```
