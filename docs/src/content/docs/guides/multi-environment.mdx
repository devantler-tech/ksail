...
- **Use `ksail workload validate`** with the target environment config before promoting manifests. See [Substitution Expansion (validate)](/guides/workload-validate/) for how Flux `${VAR}` substitutions are handled during validation.
