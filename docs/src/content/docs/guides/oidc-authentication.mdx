...
2. **Acts as an exec credential plugin** (`ksail oidc get-token`) — replacing the need for [kubelogin](https://github.com/int128/kubelogin).
...
OIDC can also be configured via CLI flags on `ksail cluster init` and `ksail cluster create`:
...
`ksail oidc get-token` implements the [Kubernetes exec credential plugin protocol](https://kubernetes.io/docs/reference/access-authn-authz/authentication/#client-go-credential-plugins). The authentication flow:
...
The `caFile` path is:
- Injected into the API server configuration (distribution-specific)
- Passed to `ksail oidc get-token` via kubeconfig exec args
