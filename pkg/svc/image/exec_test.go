...
func mockDockerStreamResponse(stdout, stderr string) dockertypes.HijackedResponse {
	...
	if stdout != "" {
		...
		header[0] = 1 // stdout
		payload := []byte(stdout)
		header[4] = byte(len(payload) >> 24) //nolint:gosec // G115: Docker stream header encoding
		header[5] = byte(len(payload) >> 16) //nolint:gosec // G115: Docker stream header encoding
		header[6] = byte(len(payload) >> 8)  //nolint:gosec // G115: Docker stream header encoding
		header[7] = byte(len(payload))       //nolint:gosec // G115: Docker stream header encoding
