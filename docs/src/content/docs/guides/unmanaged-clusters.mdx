...
In the web UI and desktop app, every row acts on its own context, even when two contexts share a cluster name. With unmanaged contexts `prod` and `kind-prod`, the `prod` row targets `prod` and the `kind-prod` row targets `kind-prod`; only when KSail manages a cluster named `prod` does that name mean `kind-prod`. A name that several contexts share and none owns outright, such as `kind-prod` and `k3d-prod` for a managed `prod`, is refused as ambiguous rather than guessed.
