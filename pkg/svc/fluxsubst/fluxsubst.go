// Package fluxsubst implements a cobra-free engine that expands Flux postBuild
// variable references (${VAR}, ${VAR:-default}, ${VAR:=default}) in Kubernetes
// YAML using type-aware placeholders derived from cached JSON schemas.
//
// It is consumed by the workload validate command so that kustomize output
// containing un-substituted Flux variables still validates against kubeconform
// without a live cluster. Schema lookups are disk-cache only (no network) to
// keep validation fast, deterministic, and offline-friendly.
package fluxsubst
...
//
// This is a round-trip-preserving splitter (documents are kept verbatim apart
