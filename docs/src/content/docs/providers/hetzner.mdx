...
| `autoscaler.node.expander` | string | `LeastWaste` | Expander strategy: `LeastWaste`, `LeastNodes`, `Random`. (`Price` is not supported for the Hetzner provider — it causes a fatal crash because the Hetzner cloud provider does not implement the pricing API.) |
