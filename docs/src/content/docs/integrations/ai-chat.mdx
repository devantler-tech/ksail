...
- [GitHub Copilot CLI](https://docs.github.com/en/copilot/concepts/agents/about-copilot-cli) must be installed and authenticated — run `copilot auth login` to sign in
...
The chat assistant uses the GitHub Copilot CLI authentication by default. Authenticate once with:
...
**Authentication errors:** Authenticate with `copilot auth login` in interactive terminals; in headless environments, set `KSAIL_COPILOT_TOKEN` or `COPILOT_TOKEN` (see [Authentication](#token-authentication-headless-environments) for details). KSail automatically retries transient auth failures (HTTP 429, 5xx responses, and network errors) up to 3 times with exponential backoff — `auth status check failed after N/3 attempts` messages indicate a persistent issue requiring action.
