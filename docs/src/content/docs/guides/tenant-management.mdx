...
If no token can be resolved through any of these sources, repo scaffolding is skipped with a warning and the command succeeds. To require a token (e.g., in CI), pass `--git-token` explicitly — an empty value causes an immediate error.
