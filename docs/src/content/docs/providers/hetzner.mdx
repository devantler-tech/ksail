...
| `autoscaler.node.expander` | string \| list | `LeastWaste` | Expander strategy: `LeastWaste`, `LeastNodes`, `Random`. Accepts either a single value or an ordered priority list (e.g. `[LeastNodes, LeastWaste]`) applied as a chain — the first expander filters node groups and each later one breaks the previous tie (upstream `--expander=least-nodes,least-waste`). (`Price` is rejected by KSail with a validation error for the Hetzner provider, because the Hetzner cloud provider does not implement the pricing API.) |
...
| `autoscaler.node.pools[].labels` | map | _(none)_ | Kubernetes node labels applied to every node in this pool (see [Per-pool labels and taints](#per-pool-labels-and-taints)). Keys must be valid Kubernetes label keys. |
| `autoscaler.node.pools[].taints[]` | list | _(none)_ | Kubernetes node taints applied to every node in this pool. Each entry has `key` (required), `value` (optional), and `effect` (`NoSchedule`, `PreferNoSchedule`, or `NoExecute`). |
