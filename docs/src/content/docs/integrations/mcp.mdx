...
import McpAvailableTools from "../../../partials/mcp-available-tools.mdx";
...
KSail can be found in the [GitHub MCP Registry](https://github.com/mcp) — search for `io.github.devantler-tech/ksail` to discover and install it directly from supported MCP clients.
...
The server listens on stdin, exposes supported KSail commands as MCP tools, and exits when the client disconnects. See [Integration Examples](#integration-examples) for typical usage patterns.

## Available Tools

{/* The tool inventory below is generated from the live command tree by go generate ./docs/... */}
...
All tools carry [MCP behavioral annotations](https://spec.modelcontextprotocol.io/specification/2025-03-26/server/tools/#tool-annotations) that allow clients to make smarter decisions:
...
:::note
`readOnlyHint: true` means the tool performs no cluster or infrastructure mutations. However, some `*_read` subcommands may still produce local side effects — for example, `cluster_read switch` updates the kubeconfig `current-context`, and `workload_read export` writes a tar archive to disk. Review the subcommand behavior before relying on this hint for auto-approval.
:::
...
For step-by-step setup instructions for each AI client — including PATH configuration, example prompts, and a tool reference — see [Using KSail with AI Assistants](/integrations/ai-mcp/).
...
See [Tool Responses](#tool-responses) for the full JSON envelope format.
...
The MCP server is complementary to the [AI Chat](/integrations/ai-chat/) feature:
...
Use MCP tool calls in automation frameworks to orchestrate sequences of KSail operations — for example, `cluster_write` (init → create) → `workload_write` (apply, then cipher_encrypt). Use the same connection pattern shown in [Custom MCP Client](#custom-mcp-client).
