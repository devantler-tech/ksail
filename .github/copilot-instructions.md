...
- **Mock Benchmark Fix**: Removed `.Times(b.N)` from mock expectations in benchmark tests (`pkg/client/helm/client_bench_test.go`) to fix incompatibility with `for b.Loop()`; `b.N` evaluates to 1 before the loop begins, so `.Times(b.N)` caused testify to fail with "called over 1 times" on subsequent iterations — omitting `.Times()` allows unlimited calls, which is the correct pattern for `for b.Loop()` benchmarks (PR #2701)
