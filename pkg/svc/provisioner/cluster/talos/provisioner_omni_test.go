...
func TestBuildOmniPatchInfos_WithPatches(t *testing.T) {
	...
	for _, p := range patches {
		...
		isValid := p.Scope == omniprovider.PatchScopeCluster ||
			p.Scope == omniprovider.PatchScopeControlPlane ||
			p.Scope == omniprovider.PatchScopeWorker
		assert.True(t, isValid, "unexpected patch scope: %v", p.Scope)
	}
}
...
func TestSaveOmniKubeconfig_NilClient(t *testing.T) {
	...
	err := provisioner.SaveOmniKubeconfigForTest(context.Background(), nilClientProv, "test-cluster")
...
func TestSaveOmniTalosconfig_NilClient(t *testing.T) {
	...
	err := provisioner.SaveOmniTalosconfigForTest(context.Background(), nilClientProv, "test-cluster")

	require.Error(t, err)
	require.ErrorIs(t, err, provider.ErrProviderUnavailable)
}

func TestSaveOmniKubeconfig_WritesFile(t *testing.T) {
	t.Parallel()

	// This test uses a nil-client provider so GetKubeconfig will fail.
	// We can only test the error path in unit tests without a real Omni connection.
	// Covered by TestSaveOmniKubeconfig_NilClient above.
	_ = t
}
