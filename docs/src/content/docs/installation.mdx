...
<Tabs>
  ...
  <TabItem label="Shell Installer">
    ...
    If Go is installed, the script uses `$GOPATH/bin`; otherwise it uses `~/.local/bin`. Set `KSAIL_INSTALL_DIR` to choose another destination or `KSAIL_VERSION` to install a specific release.

    > [!NOTE]
    > Versioned `go install github.com/devantler-tech/ksail/v7@...` is not supported. Go ignores the main-module dependency overrides that KSail currently needs for upstream compatibility and security fixes. To compile from source, use the **From Source** method below so those reviewed overrides remain active.
