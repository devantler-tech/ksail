...
All MCP tool handlers return a structured JSON envelope, making it easy to parse results programmatically:
...
````json
{
  ...
  "error": "exit status 1",
  "output": "<command stderr / captured output>"
}
````
...
| `command` | ✓ | Full command path that was executed |
| `output` | When non-empty | Captured stdout (success) or stderr (error) |
| `error` | On error only | Error message (e.g., exit status) |

## Integration Examples

For a complete setup guide with step-by-step instructions for each AI client — including PATH configuration, example prompts, and tool reference — see [Using KSail with AI Assistants](/guides/ai-mcp/).
...
Add KSail to your Cursor MCP configuration. Create or edit `.cursor/mcp.json` in your project root (project-level) or `~/.cursor/mcp.json` (global):
...
After saving, restart Cursor or reload MCP configuration from Cursor Settings → MCP.

### Windsurf

Add KSail via **Windsurf Settings → MCP → Add MCP Server**:
...
async with stdio_client(StdioServerParameters(command="ksail", args=["mcp"])) as (read, write):
    async with ClientSession(read, write) as session:
        ...
        # Response content is a structured JSON envelope
        envelope = json.loads(result.content[0].text)
        if envelope["status"] == "success":
            print(envelope.get("output", ""))
        else:
            print("Error:", envelope.get("error"), envelope.get("output", ""))
````
