...
After computing the diff, `ksail cluster update` displays a formatted **Change Summary** table with four columns (Component, Before, After, Impact). Rows are ordered by severity — 🔴 recreate-required → 🟡 reboot-required → 🟢 in-place — and only changed components are shown. Column widths adjust dynamically to the longest value in each column.
