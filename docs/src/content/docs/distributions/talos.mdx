...
On macOS, Docker runs in a Linux VM, so MetalLB virtual IPs are not directly accessible from the host. For Talos clusters using the **Docker provider** only, you can use `extraPortMappings` in `ksail.yaml` to expose container ports on the host (Hetzner and Omni Talos clusters do not use Docker port mappings):
...
On macOS, Docker runs in a Linux VM so MetalLB virtual IPs are not routable from the host. Use `extraPortMappings` in `ksail.yaml` to expose container ports directly on the host:
...
Access services at `http://localhost:8080`. Ports are bound on the first control-plane node only—in multi-control-plane clusters the remaining nodes are unaffected. See the [Declarative Configuration](/configuration/declarative-configuration/#port-mappings-docker-provider) reference for the full field specification.
