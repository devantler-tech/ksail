...
Use `--oci-path` to append a path suffix to the OCI URL (e.g., `oci://ghcr.io/my-org/my-team/manifests`). This prevents tag collisions when a tenant repo publishes both Docker images and Kubernetes manifests to the same registry path.
...
# K8s manifests are published to the same registry path)
ksail tenant create my-team --type flux \
  --sync-source oci \
  --registry oci://ghcr.io/my-org \
  --oci-path manifests
