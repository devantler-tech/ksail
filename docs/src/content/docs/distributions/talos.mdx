...
`ksail cluster update` automatically upgrades the Talos OS version when the compiled-in Talos version differs from the running cluster's version. KSail performs a rolling upgrade using the Talos LifecycleService API (Talos 1.13+):
