...
| `talos.schematicId` | string | _(empty)_ | [Talos factory](https://factory.talos.dev) schematic ID. When set, KSail automatically builds and manages a Hetzner snapshot image instead of booting from the ISO. The snapshot is deleted when `ksail cluster delete --delete-storage` is run. |
