...
[vCluster](https://www.vcluster.com/) creates virtual Kubernetes clusters. KSail uses the [Vind](https://github.com/loft-sh/vcluster) Docker driver to run the vCluster control plane (and optional worker nodes) directly as Docker containers, without requiring a pre-existing host Kubernetes cluster. This makes VCluster ideal for lightweight, isolated environments that still behave like full Kubernetes clusters while using Docker as the only infrastructure provider.

**Key characteristics:**

- Control plane and optional worker nodes run as Docker containers via the Vind driver
- No VM or physical node management — Docker manages container lifecycle
- Very fast cluster creation time with a small resource footprint
- Configured via `vcluster.yaml` and the vCluster Helm chart
