...
Cloud Provider KIND runs as an external Docker container (`ksail-cloud-provider-kind`) with per-service containers prefixed `cpk-`. It watches for LoadBalancer services and allocates IPs from the Docker bridge network, accessible from your host.
