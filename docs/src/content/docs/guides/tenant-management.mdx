...
kubectl tenants generate RBAC-only platform manifests (no GitOps sync resources). When `--git-provider github` and `--git-repo` are provided, kubectl tenants also get a scaffolded tenant repository — just without GitOps-specific resources:
