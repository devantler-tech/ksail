...
On macOS, Docker runs in a Linux VM, so MetalLB virtual IPs are not accessible from the host. Use `extraPortMappings` in `ksail.yaml` to expose container ports directly:
...
Access services at `http://localhost:8080`. Port mappings are applied only to the first control-plane node to avoid host port collisions when running multiple control-plane nodes.
