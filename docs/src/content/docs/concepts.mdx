...
[Cilium](https://cilium.io/) is an eBPF-based CNI offering networking, security, and observability with features like transparent encryption and service mesh. KSail enables Cilium's [Gateway API](https://gateway-api.sigs.k8s.io/) integration by default (`gatewayAPI.enabled: true`) and automatically pre-installs the standard [Gateway API CRDs](https://gateway-api.sigs.k8s.io/guides/) as a prerequisite. On Docker-based clusters, host network mode is configured so Gateway API routes traffic via the Docker bridge without an external load balancer. See [documentation](https://docs.cilium.io/), [concepts](https://docs.cilium.io/en/stable/overview/intro/), [Gateway API guide](https://docs.cilium.io/en/stable/network/servicemesh/gateway-api/gateway-api/), and [network policies](https://docs.cilium.io/en/stable/security/policy/).
