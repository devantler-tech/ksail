...
Encrypted secrets in your source directory are decrypted transparently during sync — no manual plugin setup required. For key resolution details and all `spec.cluster.sops` options, see [Secret Management](/features/secret-management/).
...
spec:
  cluster:
    ...
    sops:
      ...
      # enabled: true       # set explicitly to disable auto-detect
```

> [!NOTE]
> `sops.ageKeyEnvVar` is deprecated — use `sops.env.var` instead. For all `spec.cluster.sops` options, see [Declarative Configuration](/configuration/declarative-configuration/).
