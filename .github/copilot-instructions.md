...
- **Cluster Switch Command**: Added `ksail cluster switch <cluster-name>` command (`pkg/cli/cmd/cluster/switch.go`) to switch the active kubeconfig context; automatically resolves the cluster name to the correct context by trying all supported distribution prefixes (`kind-`, `k3d-`, `admin@`, `vcluster-docker_`); returns an error if multiple matching contexts are found; kubeconfig is resolved from `KUBECONFIG` env var, `ksail.yaml`, or `~/.kube/config` in that order; supports shell completion via `listClusterNames()` (PR #2804)
- **Registry Container Healthcheck**: Added Docker-native HEALTHCHECK to all registry containers (both local and mirror registries) in `pkg/client/docker/registry_container.go`; uses `wget -q --spider http://localhost:5000/v2/` (Alpine-compatible) every 10s, marking the container unhealthy after 3 consecutive failures; healthcheck state is visible via `docker ps` and `docker inspect`; uses `DefaultRegistryPort` constant rather than a hardcoded port (PR #2820)
- **OCI Push Retry**: Added `pushWithRetry` in `pkg/client/oci/builder_impl.go` to retry OCI image pushes on transient network errors; uses `netretry.IsRetryable` and `netretry.ExponentialDelay` (3 attempts, 2s base, 10s max backoff); wraps errors with `"push failed (non-retryable): ..."` for immediate failures and `"push failed after N attempts: ..."` for exhausted retries; mirrors the existing `verifyWithRetry` pattern in `pkg/client/oci/verifier.go` (PR #2821)
- **Hetzner Provider Split**: Split `pkg/svc/provider/hetzner/provider.go` (844 lines) into three focused files — `server_opts.go` (`CreateServerOpts` struct + `buildServerCreateOpts` builder, 83 lines), `server_retry.go` (`ServerRetryOpts` + `CreateServerWithRetry` + retry helpers, logging helpers, and `shouldDisable`/`shouldRetry` predicates, 242 lines), and `provider.go` now focuses on lifecycle and low-level helpers (506 lines, −40%); removed 12 `//nolint:funcorder` suppressions (PR #2836)
- **Update Diff Display Enhancement**: Added `--yes`/`-y` flag to `ksail cluster update` as an alias for `--force` to skip the recreate confirmation prompt; enhanced the diff change listing to show `old → new` values alongside the emoji severity icon for each changed field (PR #2834)
- **Visual Before/After Diff Table in `ksail cluster update`**: Added `displayChangesSummary()` in `pkg/cli/cmd/cluster/update.go` that renders a formatted before/after table titled "🔍 Change summary" with four columns — Component (with severity icon), Before, After, Impact — ordered by severity (🔴 recreate-required → 🟡 reboot-required → 🟢 in-place); displayed during `--dry-run` and after successful update when changes exist; column widths auto-sized; implemented with `strings.Builder` and `Grow()` pre-allocation (PR #2858)

- **VSCode Cluster Status View**: Added real-time cluster status sidebar view (`vsce/src/views/clusterStatusView.ts`) and compact status bar indicator (`vsce/src/views/statusBar.ts`) to the VSCode extension (PR #2859); the sidebar polls every 10 seconds (configurable via `ksail.statusPollingInterval`) and shows cluster health (Healthy/Degraded/Error/Unknown), pod summaries grouped by namespace, and GitOps reconciliation state; clicking a failed/pending pod opens its logs in an output channel; the status bar item uses a color-coded indicator/background for health state and is clickable to manually refresh
