...
Before submitting, KSail checks the effective credentials of both the EKS and STS
clients, including service-specific overrides. Each must cover the wait deadline
...
refresh into a different identity. The checked credentials sign the final
cluster check, the update and its wait, and are released when the upgrade
ends; a second upgrade cannot start on a client that still holds them.
Reusing a captured selection preserves its expiry and endpoint settings;
node-group inventory also retains the captured CloudFormation endpoint if the
