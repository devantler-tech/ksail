...
> [!NOTE]
> Automated CI tests don't need `--ttl` — create the cluster, run tests, and delete it explicitly. See [Testing in CI/CD](/guides/cicd-integration/) for that pattern. However, `--ttl` can be added as a safety net in CI to auto-destroy clusters if the cleanup step is skipped (e.g. runner crash).
