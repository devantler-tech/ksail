...
1. **Hetzner Cloud Firewall** — Restricts external traffic at the network perimeter. KSail manages three rules: Talos API (50000/tcp), Kubernetes API (6443/tcp), and Talos Kubespan (51820/udp). Ports for cluster-internal communication (etcd, kubelet, trustd) are not exposed because nodes communicate over the private Hetzner Cloud Network.
...
> [!NOTE]
> **Upgrading from older KSail versions**: Hetzner Cloud Firewall rules were reduced from 6 to 3 in KSail v0.x (etcd, kubelet, and trustd rules removed). Running `ksail cluster update` automatically migrates existing clusters to the new rule set.
