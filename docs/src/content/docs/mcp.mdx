...
| Tool | Subcommands |
|------|-------------|
| `cluster_read` | `info`, `list`, `connect`, `switch` (pass cluster name via `args`; mutates local kubeconfig current-context), `diagnose` (pass `--format json` via `args` for structured output) |
