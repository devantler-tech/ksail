...
- VCluster (Vind) runs control-plane and optional worker nodes as Docker containers — CNI, CSI, metrics-server, and LoadBalancer are managed by the vCluster Helm chart and are N/A for update operations; `spec.cluster.loadBalancer` has no effect on VCluster and changing it will not trigger a cluster update
