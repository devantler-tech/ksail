...
> [!NOTE]
> The `talosconfig` file is automatically created in the project directory during cluster creation.
...
3. **Use explicit node IP:**
   ```bash
   talosctl -n <node-ip> --talosconfig=./talosconfig get members
   ```
...
Specify a specific Talos version:
...
spec:
  cluster:
    distribution: Talos
    version: "v1.6.0"  # Specific Talos version
```
...
- **Explore [LoadBalancer Configuration](/configuration/loadbalancer/)** to understand LoadBalancer options for Talos
- **Read [Configuration Reference](/configuration/)** for complete ksail.yaml options
- **Check [Support Matrix](/support-matrix/)** for detailed Talos × component compatibility
- **Learn about [Secret Management](/features/#secret-encryption)** with SOPS for encrypting Talos configs
- **Review [Talos Documentation](https://www.talos.dev/latest/)** for deep dive into Talos Linux
