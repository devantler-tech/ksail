...
description: Deploy production-ready Talos, K3s, or Vanilla (kubeadm) Kubernetes clusters on Hetzner Cloud servers with KSail.
---

The Hetzner provider creates Kubernetes cluster nodes as [Hetzner Cloud](https://www.hetzner.com/cloud/) servers. Nodes run Talos Linux (the default and most battle-tested path), or a standard Linux image bootstrapped into a native **K3s** or **Vanilla (kubeadm)** cluster via cloud-init. It provisions real cloud infrastructure with public IPs, private networking, load balancers, and persistent volumes — ideal for production-grade clusters and cloud testing.
...
- Deploy clusters with high-availability placement groups

> [!NOTE]
> The Hetzner provider supports the **Talos**, **K3s**, and **Vanilla (kubeadm)** distributions. Talos is the default and most battle-tested path; the K3s and Vanilla paths bootstrap standard Linux servers via cloud-init (see [K3s and Vanilla clusters](#k3s-and-vanilla-kubeadm-clusters) below). For local Docker-based clusters, see the [Docker Provider](/providers/docker/) or the [Talos distribution guide](/distributions/talos/).
...
3. **Talos ISO or Schematic** *(Talos distribution only)* — Either KSail's automatically tracked default x86 Talos ISO must be available in your Hetzner Cloud project (for ARM, look up the matching ISO ID in the Hetzner Cloud Console), or a [Talos factory schematic ID](https://factory.talos.dev) can be provided via `spec.cluster.talos.schematicId` so KSail builds the snapshot automatically. See [Talos options](/configuration/declarative-configuration/#distribution-and-tool-options).
...
spec:
  cluster:
    ...
    workers: 2
    talos:
      # Omit ISO to use KSail's automatically tracked Hetzner x86 default.
      # For ARM, set the matching ISO ID from the Hetzner Cloud Console.
      # A custom ISO also requires version to match that ISO's Talos release.
  ...
  provider:
    hetzner:
      ...
      # Restrict public Kubernetes API (6443) access on control planes and public Talos API (50000) access on every node to these CIDRs.
      # Private worker Talos API access from the cluster network remains enabled.
...
The full `spec.provider.hetzner` field reference — every option with its type, default, and
description — is generated from the API types: see
[spec.provider.hetzner (OptionsHetzner)](/configuration/declarative-configuration/#specproviderhetzner-optionshetzner)
in the Declarative Configuration reference.

## K3s and Vanilla (kubeadm) Clusters

Beyond Talos, the Hetzner provider can bootstrap **native K3s** and **Vanilla (kubeadm)**
clusters. Instead of a Talos ISO, KSail creates servers from a standard Linux image and
delivers the whole node bootstrap declaratively through **cloud-init** user data — the k3s
install (server-init → additional servers → agents) or the kubeadm init/join sequence runs on
first boot, and KSail fetches the kubeconfig once the control plane is up. No Talos ISO or
schematic is needed, and all `spec.provider.hetzner` options above (server types, location,
private networking, placement groups, firewalls, IPv4-less nodes) apply the same way:
...
spec:
  cluster:
    distribution: K3s      # or Vanilla for a kubeadm cluster
    provider: Hetzner
    controlPlanes: 1
    workers: 2
  provider:
...
Differences from the Talos path to be aware of:

- **K3s** bundles its usual defaults — the local-path storage provisioner and the servicelb
  load balancer — so PersistentVolumeClaims and `LoadBalancer` Services work out of the box
  (servicelb exposes node IPs rather than provisioning Hetzner Cloud load balancers).
- **Vanilla (kubeadm)** provisions a bare cluster: bring your own CSI and load balancer, as
  on any self-managed kubeadm install.
- Node OS management (upgrades, hardening) is yours on the K3s/Vanilla paths; Talos handles
  it declaratively via its own API.
...
| --- | --- | --- | --- |
| `talos.iso` | int | tracked x86 default | Hetzner Cloud ISO/image ID for Talos Linux (look up the matching ARM ID in the Hetzner Cloud Console). A custom ID requires `talos.version` to match the ISO's Talos release. Ignored when `schematicId` is set. |
...
#### Using a custom Talos ISO

When you select a custom ISO, including an ARM ISO, set `spec.cluster.talos.version`
to the Talos release that ISO boots. KSail cannot infer a release from a Hetzner ISO
ID and refuses to generate machine configuration for an unpinned custom ISO. This
prevents a newer default version contract from adding fields an older ISO cannot read.

```yaml
# Partial snippet — replace both values with your ISO's ID and Talos release.
spec:
  cluster:
    distribution: Talos
    provider: Hetzner
...
You can also provide the version per invocation, for example
`ksail cluster create --distribution-version v1.12.4`. Omitting `talos.iso`, or using
KSail's tracked default ISO ID, keeps the default version contract. A schematic
snapshot also requires a version pin and takes precedence over the ISO.
...
The full autoscaler field reference (`spec.cluster.autoscaler`, including node pools and per-pool
labels/taints) is generated from the API types: see
[spec.cluster.autoscaler (AutoscalerConfig)](/configuration/declarative-configuration/#specclusterautoscaler-autoscalerconfig)
...
> [!NOTE]
> The label is applied via the autoscaler worker's Talos `machine.nodeLabels` (the kubelet `--node-labels` flag), so it lands on the real `Node` object. The upstream Hetzner cluster-autoscaler deliberately does **not** push its per-pool `nodeConfigs[].labels`/`taints` to the kubelet — those only seed the in-memory template node used for scheduling simulation and scale-from-zero (see [kubernetes/autoscaler#8492](https://github.com/kubernetes/autoscaler/issues/8492), closed as working-as-intended). Stamping the label in the worker cloud-init is therefore the canonical mechanism.
...
2. **Talos OS-level ingress firewall** — Defense-in-depth at the node level, enabled by default (`spec.provider.hetzner.ingressFirewall: Enabled`). Blocks all ingress by default; opens only the ports required for each node role (control plane vs worker), restricted to the cluster subnet where appropriate. When `allowedCidrs` is set, the public Kubernetes API and Talos API `NetworkRuleConfig` rules restrict public source IPs to those CIDRs while retaining worker Talos API access from the private cluster subnet. Disable with `ingressFirewall: Disabled` if you manage node-level firewall rules yourself.
...
- **Create** — `ksail cluster create` calls the [Talos factory](https://factory.talos.dev) to download the raw disk image for the specified schematic and Talos version, uploads it to Hetzner Cloud as a snapshot, and labels it with `ksail.io/talos-version`, `ksail.io/talos-schematic`, `ksail.io/cluster`, and `ksail.io/snapshot-build`. If a matching snapshot already exists, it is reused.
- **Build server** — The upload runs on a temporary `hcloud-upload-image-*` server that is deleted once the snapshot exists. If the build is interrupted (Ctrl-C or `SIGTERM`), KSail deletes that server and its SSH key before exiting.
