...
- **¹⁰ EKS (Planned)**: Full component installer support for EKS is in progress. `ksail project init` is available, but `ksail cluster create` is not yet functional. See [EKS Distribution](/distributions/eks/) for details.
