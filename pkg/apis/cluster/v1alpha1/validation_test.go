...
func TestValidateHetznerPublicNet(t *testing.T) {
	t.Parallel()

	disabled := false
	bothOff := &v1alpha1.OptionsHetzner{WorkerPublicIPv4: &disabled, WorkerPublicIPv6: &disabled}
	cpOff := &v1alpha1.OptionsHetzner{
		ControlPlanePublicIPv4: &disabled,
		ControlPlanePublicIPv6: &disabled,
	}
	ipv4Off := &v1alpha1.OptionsHetzner{WorkerPublicIPv4: &disabled}
	...
	tests := []struct {
		...
		{
			name:              "DefaultsAreReachable",
			hetzner:           &v1alpha1.OptionsHetzner{},
			hasPrivateNetwork: true,
		},
		{name: "WorkerFullyPrivateWithNetworkAllowed", hetzner: bothOff, hasPrivateNetwork: true},
		{name: "WorkerFullyPrivateNoNetworkRejected", hetzner: bothOff, wantErr: true},
		{name: "ControlPlaneFullyPrivateNoNetworkRejected", hetzner: cpOff, wantErr: true},
		{name: "WorkerIPv4OnlyDisabledNoNetworkAllowed", hetzner: ipv4Off},
	}
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			err := v1alpha1.ValidateHetznerPublicNet(testCase.hetzner, testCase.hasPrivateNetwork)
			if testCase.wantErr {
				require.ErrorIs(t, err, v1alpha1.ErrHetznerNodeUnreachable)
			} else {
				require.NoError(t, err)
			}
		})
