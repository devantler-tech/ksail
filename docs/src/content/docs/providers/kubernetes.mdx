...
> **Note:** Kubernetes provider cluster listing returns an empty result in the current version. Use `kubectl get namespaces -l ksail.io/managed-by=ksail` on the host cluster to find active nested clusters.
