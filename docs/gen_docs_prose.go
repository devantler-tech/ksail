...
KSail also checks the file when a command loads it and reports its progress: a key it does not recognise — a misspelling, or a setting nested at the wrong level — is reported as a warning that names the key's path and, when one is close, the key you probably meant. KSail ignores that key, so rename or remove it. KSail reads only the first YAML document in the file, so it also warns when another document with content follows it. Commands that load the configuration quietly, without printing load progress, skip these checks.`
